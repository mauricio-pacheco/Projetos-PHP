...
error_reporting(E_ALL ^ E_NOTICE);
	...
	if (substr($TemplateGalleryPath,strlen($TemplateGalleryPath)-(1))!="/")
		$TemplateGalleryPath=$TemplateGalleryPath."/";	
	
	if($AbsoluteTemplateGalleryPath!="")	
	{
		if (substr($AbsoluteTemplateGalleryPath,strlen($AbsoluteTemplateGalleryPath)-(1))!="/")
			$AbsoluteTemplateGalleryPath=$AbsoluteTemplateGalleryPath."/";
	}
		
	if(str_contains($current_Path, $TemplateGalleryPath))
	{
	}
	else
	{
	...
	else
	{
		...
		{
			case "deletefile":
				unlink(ServerMapPath($_GET["filename"],$AbsoluteTemplateGalleryPath,$TemplateGalleryPath));
				print "<script language=\"javascript\">parent.ResetFields();</script>";
				break;
			case "renamefile":
			...
			case "renamefolder":
				...
				break;
			case "deletefolder":
				rmdir(ServerMapPath($_GET["foldername"],$AbsoluteTemplateGalleryPath,$TemplateGalleryPath)); 
				break;
			case "createfolder":
				$folderPath=$_GET["foldername"];
				$folderPath=$current_Path.$folpath.$folderPath;
				$folderPath=ServerMapPath($folderPath,$AbsoluteTemplateGalleryPath,$TemplateGalleryPath);
	...
	function Showbrowse_Img($spec)
	{
	  extract($GLOBALS);
	  $s="";
	  $directory=ServerMapPath($spec,$AbsoluteTemplateGalleryPath,$TemplateGalleryPath);
...
$str_openfolderEvent="onclick=\"parent.SetUpload_FolderPath('".$current_Path.$folpath.$file."');location.href='browse_Template.php?".$setting."&loc=".$folpath.$file."&Theme=".$Theme."&MP=".$current_Path."';\"";
		    $s=$s."<tr onMouseOver=\"row_over(this)\" onMouseOut=\"row_out(this)\" class='cursor' class='cursor'>>";
    ...
    if($filelist)
		{
		    ...
		    $s=$s."<td><img src=\"../Images/".GetExtension($file).".gif\"></td>"."\r\n";
		    $s=$s."<td>"."\r\n";
		    $s=$s.$file."&nbsp;</td>"."\r\n";
...
<html>
  <head>
    ...
    <script type="text/javascript">
      var folderpath = "browse_Template.php?<?php echo $setting ; ?>&MP=<?php echo $current_Path ; ?>&Theme=<?php echo $Theme; ?>";
      ...
      for (i=0;i<arrloc.length-2;i++) {
			...
			str="browse_Template.php?<?php echo $setting ; ?>&MP=<?php echo $current_Path ; ?>&Theme=<?php echo $Theme; ?>&loc=" + str + "&u=y";
			location.href = str;
