...
$file = $_FILES['photoupload']['tmp_name'];
	...
	elseif (!$error && ($size[0] < 25) || ($size[1] < 25))
	{
		...
		$aux_tipo_imagem = $size['mime'];
            ...
            $diretorio = "../ups/fotosnot/";
            $diretorio_g = "../ups/fotosnot/g/";
            $diretorio_p = "../ups/fotosnot/p/";
            ...
            if(!file_exists($diretorio_p)) {
				...
				// foto grande
					...
					$ft_nome2 = "imagem_".date('dmy').time().".png";
					$larg_ft = 980;
					$altu_ft = 270;
				// foto minuatura
					$tb_nome = "thumb_".date('dmy').time()."";	
					$tb_nome2 = "thumb_".date('dmy').time().".png";	
					$larg_tb = 96;
					$altu_tb = 72;
	...
	else
	{
		...
		include "../conexao.php";
		$sql = "INSERT INTO cw_anexosnot (fotomaior, fotomenor, idnot) VALUES ('$ft_nome2','$tb_nome2','$idpuxado')";
		mysql_query($sql);
	}
