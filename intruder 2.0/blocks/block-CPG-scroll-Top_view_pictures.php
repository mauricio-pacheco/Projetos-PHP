...
// ------------------------------------------------------------------------- //
if (eregi("block-CPG-scroll-Top_view_pictures.php", $_SERVER['PHP_SELF'])) {
...
while ($row = $db->sql_fetchrow($result)) {
    ...
    if ($CONFIG['thumb_method']=='none'){
	    $content .= '<a href="' . $CPG_M_URL . '&file=displayimage&pos=-' . $row["pid"] . '"><img src="' . $row["filepath"] . $row["filename"] . '" border="0" alt="' . $thumb_title . '" title="' . $thumb_title . '" width="'. $CONFIG['thumb_width'] .'"><br>' . $views_title . '</a><br /><br />';
    }else{
	    $content .= '<a href="' . $CPG_M_URL . '&file=displayimage&pos=-' . $row["pid"] . '"><img src="' . $row["filepath"] . $CONFIG["thumb_pfx"] . $row["filename"] . '" border="0" alt="' . $thumb_title . '" title="' . $thumb_title . '"><br>' . $views_title . '</a><br /><br />';
    }
} 
