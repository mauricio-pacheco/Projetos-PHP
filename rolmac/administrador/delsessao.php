          ...
          <table width="100%" border="0" align="center">
            <tr>
              <td width="11%" height="30" bgcolor="#076CA0"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td width="98%">&nbsp;&nbsp;<font color="#FFFFFF" class="fontetabela2"><b>APAGAR SESSÃO</b></font></td>
                </tr>
              </table></td>
            </tr>
          </table>
...
$sql = "DELETE FROM cw_sessoes WHERE id='$id'";

if(mysql_query($sql)) {
echo "<script>alert('A SESSÃO FOI APAGADA COM SUCESSO!')</script>";
echo "<script>location.href='admsessoes.php?id=$id'</script>";
}else{
echo "<div align=center><br><font color='#000000' >NÃO FOI POSSÍVEL APAGAR!</font></div>";
}

$sql3 = mysql_query("SELECT * FROM cw_pub WHERE idsessao='$id' ORDER BY id");
while($y = mysql_fetch_array($sql3))
{
$arquivo = $y["arquivo"];
unlink("publicacoes/$arquivo");
}

$sql2 = "DELETE FROM cw_subsessoes WHERE idsessao='$id'";
mysql_query($sql2);

$sqlnovo = "DELETE FROM cw_pub WHERE idsessao='$id'";
mysql_query($sqlnovo);
