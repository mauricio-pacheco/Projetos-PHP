	...
	case 'startCalendar':
		...
		echo '<div style="margin-bottom: 0px;">
					<form name="changeCalendarDate">
						<select class="manchete_texto999" class=caixacontato id="ccMonth" onChange="startCalendar($F(\'ccMonth\'), $F(\'ccYear\'))">';
	...
	case 'listEvents':
		...
		if($eventQuery) {
			if(mysql_num_rows($eventQuery) >0) {
				echo '* OS EVENTOS POSTADOS ABAIXO AINDA NÃO ESTÃO CONFIRMADOS, PARA MAIS INFORMAÇÕES ENTRE EM CONTATO CONOSCO <a href=contato.php class=manchete_texto99>CLICANDO AQUI</a><br><br><font color=#FBED00><b>EVENTOS DO DIA:</b></font>&nbsp; '. date("d", $timeStamp) .'/'. date("m", $timeStamp) .'/'. date("Y", $timeStamp) .'<br><br>';
