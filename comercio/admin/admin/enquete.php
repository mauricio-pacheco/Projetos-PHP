...
window.open(link,'Janela', 'top=0,left=0,toolbar=no, location=no, status=no, scrollbar=no,resizable=yes,width=450, height=380')
}
function mostra(link){
window.open(link,'Janela', 'top=0,left=0,toolbar=no, location=no, status=no, scrollbar=no,resizable=yes,width=450, height=400')
}
</Script>
...
?></td>
    ...
    <td width="80%" height="80%" valign="top">
     <blockquote>
      ...
      <p><font size="2" face="Verdana, Arial, Helvetica, sans-serif"><b>Manuten&ccedil;&atilde;o de enquetes.</b></font></p>
      <p><font size="1" face="Verdana, Arial, Helvetica, sans-serif"><a class=link_verde href="javascript:abrejanela('ad_enquete_form.php');"><b>Cria nova Enquete</b> - Clique aqui para adicionar uma enquete.</a><br>
      <p><font size="1" face="Verdana, Arial, Helvetica, sans-serif"><br><strong>Enquetes cadastradas:</strong></font>
      <p><font color="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif">
        ...
        include "../db.php";
        /// Incia paginação
        $sql_paginacao = "SELECT * FROM enquete ORDER BY id ASC LIMIT $inicio,$max";
        $sql_paginacao_1 = "SELECT * FROM enquete ORDER BY id ASC";
        $max = $qtde_p_p_p;
        include ("paginacao.php");
        /// Termina paginação
        $sql = mysql_query("SELECT * FROM enquete ORDER BY id ASC LIMIT $inicio,$max");
        while($ressql = mysql_fetch_array($sql)){
        print "<a class=link_verde href=\"javascript:mostra('mostra_enquete.php?id=$ressql[0]');\">$ressql[0] - $ressql[1]</a><br>";
        }
        ?>
      </font></p>
