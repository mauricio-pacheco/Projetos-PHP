    ...
    <td width="80%" height="80%" valign="top">
      <blockquote>
        ...
        <p><font size="1" face="Verdana, Arial, Helvetica, sans-serif"><b>Manuten&ccedil;&atilde;o
          geral dos usu&aacute;rios do boletim.</b></font></p>
        <p><font color="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif"><?
$path_local = "../padrao.php";
include "../db.php";
/// Se a categoria for maior que a principal tenta paginar Produtos.
/// Incia paginação
$sql_paginacao = "SELECT * FROM boletim ORDER BY email ASC LIMIT $inicio,$max";
$sql_paginacao_1 = "SELECT * FROM boletim ORDER BY email ASC";
$max = $qtde_p_p_p;
include ("paginacao.php");
/// Termina paginação
$sql = mysql_query("SELECT * FROM boletim ORDER BY email ASC LIMIT $inicio,$max");
while($ressql = mysql_fetch_array($sql)){
print "<a class=link_verde href=\"javascript:alteraproduto('al_boletim_form.php?id=$ressql[0]');\">$ressql[2]</a><br>";
}
?></font></p>
