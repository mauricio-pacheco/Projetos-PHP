...
//	require("getset.php");
    ...
    class HTTP_WebDAV_Server_Filesystem extends HTTP_WebDAV_Server 
    {
        ...
        function fileinfo($uri, $options) 
        {
            ...
            $query = "SELECT ns, name, value FROM $GLOBALS[TBL_PREFIX]webdav_properties WHERE path = '$uri'";
...
//            $stat = mkdir ($parent."/".$name,0777);
        ...
        function delete($options) 
        {
            ...
            if (is_dir($path)) {
                $query = "DELETE FROM $GLOBALS[TBL_PREFIX]webdav_properties WHERE path LIKE '$options[path]%'";
                mysql_query($query);
                system("rm -rf $path");
            } else {
                unlink ($path);
            }
            $query = "DELETE FROM $GLOBALS[TBL_PREFIX]webdav_properties WHERE path = '$options[path]'";
            mysql_query($query);
        ...
        function copy($options, $del=false) 
        {
            ...
            if (is_dir($source)) {
                ...
                if($del) {
                    @unlink($dest);
                    $query = "DELETE FROM $GLOBALS[TBL_PREFIX]webdav_properties WHERE path = '$options[dest]'";
                    mysql_query($query);
                    rename($source, $dest);
                    $query = "UPDATE $GLOBALS[TBL_PREFIX]webdav_properties SET path = '$options[dest]' WHERE path = '$options[path]'";
                    mysql_query($query);
                } else {
        ...
        function proppatch(&$options) 
        {
            ...
            foreach($options["props"] as $key => $prop) {
                if($ns == "DAV:") {
                    ...
                    if(isset($prop["val"])) {
                        $query = "REPLACE INTO $GLOBALS[TBL_PREFIX]webdav_properties SET path = '$options[path]', name = '$prop[name]', ns= '$prop[ns]', value = '$prop[val]'";
                    } else {
                        $query = "DELETE FROM $GLOBALS[TBL_PREFIX]webdav_properties WHERE path = '$options[path]' AND name = '$prop[name]' AND ns = '$prop[ns]'";
                    }       
                    mysql_query($query);
                }
            }
        ...
        function lock(&$options) 
        {
            if(isset($options["update"])) { // Lock Update
                $query = "UPDATE $GLOBALS[TBL_PREFIX]webdav_locks SET expires = '".(time()+300)."' where token='" . $options[locktoken] . "'";
            ...
            $query = "INSERT INTO $GLOBALS[TBL_PREFIX]webdav_locks
        ...
        {
            $query = "DELETE FROM $GLOBALS[TBL_PREFIX]webdav_locks WHERE path = '$options[path]' AND token = '$options[token]'";
            mysql_query($query);
        ...
        function checklock($path) 
        {
            ...
            $query = "SELECT owner, token, expires, exclusivelock
                  FROM $GLOBALS[TBL_PREFIX]webdav_locks
                 WHERE path = '$path'
               ";
            $res = mysql_query($query);
