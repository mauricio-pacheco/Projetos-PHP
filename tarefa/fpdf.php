...
function Output($file='',$download=false)
{
	...
	global $HTTP_ENV_VARS, $pdf_filename;
	
	if ($pdf_filename == "") {
			$pdf_filename = "CRM-CTT_PDF_Export.pdf";
	}
...
function FancyTable4colSinglePDF($header,$data)
{
    ...
    foreach($data as $row)
    {
		...
		$this->Cell($w[4],6,$row[4],'LR',0,'L',$fill,$row[3]);
        $this->Ln();
        $fill=!$fill;
    }
...
function FancyTable2col($header,$data)
{
//		return(1);
    //Colors, line width and bold font
    $this->SetFillColor(128,0,0);
    ...
    $this->Cell(0,3,$header[1],1,0,'C',1);
    ...
    foreach($data as $row)
    {
        ...
        $this->Cell(0,3,$row[1],'LR',0,'L',$fill);
        $this->Ln();
        $fill=!$fill;
    }
    $this->Cell(0,0,'','T');
}
function FancyTable3col($header,$data)
{
    //Colors, line width and bold font
    $this->SetFillColor(128,0,0);
    $this->SetTextColor(255);
    ...
    foreach($data as $row)
    {
		if ($row[3]<>"" && $row[4]<>"" && $row[5]<>"") {
			    $this->SetFillColor($row[3], $row[4], $row[5]);
				if ($fill==0) {
					$fill=1;
					$fillset=1;
				}
...
function FancyTable5col($header,$data)
{
    //Colors, line width and bold font
    $this->SetFillColor(128,0,0);
    $this->SetTextColor(255);
...
function FancyTable5colSpecial($header,$data)
{
    //Colors, line width and bold font
    $this->SetFillColor(128,0,0);
    $this->SetTextColor(255);
...
function FancyTable4colWide($header,$data)
{
    //Colors, line width and bold font
    $this->SetFillColor(128,0,0);
    $this->SetTextColor(255);
...
function FancyTable4colHops($header,$data)
{
    //Colors, line width and bold font
    $this->SetFillColor(128,0,0);
    $this->SetTextColor(255);
...
class PDF_AutoPrint extends PDF_Javascript
{
	...
	function CloseDoc()
	{
		//Embed some JavaScript to close the document
//		$param=($dialog ? 'true' : 'false');
	//	$script="CloseDoc();";
//		$this->IncludeJS($script);
	}
