          ...
          <table width="100%" border="0" align="center">
            <tr>
              <td width="11%" height="30" bgcolor="#076CA0"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td width="98%">&nbsp;&nbsp;<font color="#FFFFFF" class="fontetabela2"><b>APAGAR NOTÍCIA</b></font></td>
                </tr>
              </table></td>
            </tr>
          </table>
...
$sql = "DELETE FROM cw_noticias WHERE id='$id'";
if(mysql_query($sql)) {
echo "<div align=center class=manchete_texto><br>A NOT&Iacute;CIA FOI APAGADA COM SUCESSO!!</div>";
unlink("ups/capasnoticia/$foto");
echo "<script>alert('NOTÍCIA APAGADA COM SUCESSO!')</script>";
echo "<script>location.href='admsnot.php'</script>";
}else{
echo "<div align=center><br>N&Atilde;O FOI POSS&Iacute;VEL APAGAR A NOT&Iacute;CIA!</div>";
}

$sql3 = mysql_query("SELECT * FROM cw_anexosnot WHERE idnot='$id' ORDER BY id");
while($y = mysql_fetch_array($sql3))
{
$fotomaior = $y["fotomaior"];
$fotomenor = $y["fotomenor"];
unlink("fotosnot/g/$fotomaior");
unlink("fotosnot/p/$fotomenor");
}

$sql2 = "DELETE FROM cw_anexosnot WHERE idnot='$id'";
mysql_query($sql2)
