...
error_reporting(E_ALL ^ E_NOTICE);
	...
	if (substr($MediaGalleryPath,strlen($MediaGalleryPath)-(1))!="/")
		$MediaGalleryPath=$MediaGalleryPath."/";	
	
	if($AbsoluteMediaGalleryPath!="")	
	{
		if (substr($AbsoluteMediaGalleryPath,strlen($AbsoluteMediaGalleryPath)-(1))!="/")
			$AbsoluteMediaGalleryPath=$AbsoluteMediaGalleryPath."/";
	}
	...
	else
	{
		...
		{
			case "deletefile":
				unlink(ServerMapPath($_GET["filename"],$AbsoluteMediaGalleryPath,$MediaGalleryPath));
				print "<script language=\"javascript\">parent.ResetFields();</script>";
				break;
			case "renamefile":
				rename(ServerMapPath($_GET["filename"],$AbsoluteMediaGalleryPath,$MediaGalleryPath),ServerMapPath($_GET["newname"],$AbsoluteMediaGalleryPath,$MediaGalleryPath));
			...
			case "renamefolder":
				...
				break;
			case "deletefolder":
				rmdir(ServerMapPath($_GET["foldername"],$AbsoluteMediaGalleryPath,$MediaGalleryPath)); 
				break;
			case "createfolder":
				$folderPath=$_GET["foldername"];
				$folderPath=$current_Path.$folpath.$folderPath;
				$folderPath=ServerMapPath($folderPath,$AbsoluteMediaGalleryPath,$MediaGalleryPath);
	...
	function Showbrowse_Img($spec)
	{
	  extract($GLOBALS);
	  $s="";
	  $directory=ServerMapPath($spec,$AbsoluteMediaGalleryPath,$MediaGalleryPath);
...
$s=$s."<th width=21><img src=\"../Images/refresh.gif\" title=\"refresh\" onclick=\"parent.Refresh('".$folpath."');\" onMouseOver=\"parent.CuteEditor_ColorPicker_ButtonOver(this);\" class=dialogButton></th>";
    ...
    if($dirlist)
        ...
        //add the html for the folders
		    $str_openfolderEvent="onclick=\"parent.SetUpload_FolderPath('".$current_Path.$folpath.$file."');location.href='browse_Media.php?".$setting."&loc=".$folpath.$file."&Theme=".$Theme."&MP=".$current_Path."';\"";
...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <script type="text/javascript">
      var folderpath = "browse_Media.php?<?php echo $setting ; ?>&MP=<?php echo $current_Path ; ?>&Theme=<?php echo $Theme; ?>";
      ...
      for (i=0;i<arrloc.length-2;i++) {
			...
			str="browse_Media.php?<?php echo $setting ; ?>&MP=<?php echo $current_Path ; ?>&Theme=<?php echo $Theme; ?>&loc=" + str + "&u=y";
			location.href = str;
