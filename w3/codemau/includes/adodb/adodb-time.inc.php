...
Since 0.04, we handle this correctly, so:

mktime(0,0,0,10,15,1852) - mktime(0,0,0,10,4,1852) == 24 * 3600 (1 day)
...
mktime uses invalid dates around 5-14 Oct 1582.

- 27 Jan 2003 0.03

Fixed some more month problems due to gmt issues. Added constant ADODB_DATE_VERSION.
Fixed calculation of days since start of year for <1970. Released with ADOdb 3.10.
...
/*
	Version Number
*/
define('ADODB_DATE_VERSION',0.04);
...
function adodb_date_test()
{
	...
	print adodb_mktime(0,0,0,10,5,1852);
	$diff = (adodb_mktime(0,0,0,10,15,1852) - adodb_mktime(0,0,0,10,4,1852));
	if ($diff != 3600*24) 
		print " Error in gregorian correction = $diff<br>";
		
	print " 15 Oct 1582, Fri=".(adodb_dow(1582,10,15) == 5 ? 'Fri' : 'Error')."<br>";
	print " 4 Oct 1582, Thu=".(adodb_dow(1582,10,4) == 4 ? 'Thu' : 'Error')."<br>";
...
/**
 checks for leap year, returns true if it is. No 2-digit year check
*/
...
function _adodb_getdate($origd=false,$fast=false,$is_gmt=false)
{
	...
	if ($d < -3698899200) $d -= 86400*10; // if 15 Oct 1582 or earlier, gregorian correction 
...
function adodb_mktime($hr,$min,$sec,$mon,$day,$year,$is_dst=false,$is_gmt=false) 
{
	...
	if ($year >= 1970) {
		...
		if ($ret < -3699763200 ) $ret += 10*86400; // if earlier than 5 Oct 1582 - gregorian correction
		else if ($ret < -3698899200) $ret = -3698899200; // if in limbo, reset to 15 Oct 1582.
	} 
