...
function defaultDisplay($type) {
	...
	if($postdirect == "1") {
		...
		echo "<font size=\"2\"><i>"._CALSUBMITADVICENOMOD."</i></font></center><br>";
	} else {
		echo "<center><font size=\"4\"><b>"._CALSUBMITNAME."</b></font><br><br>";
		echo "<font size=\"2\"><i>"._CALSUBMITADVICE."</i></font></center><br>";
	}
	CloseTable();
	echo "<br>";
	OpenTable();
	if (is_user($user)) getusrinfo($user);
	echo "<p><form enctype='multipart/form-data' onSubmit=\"return verify();\" action=\"modules.php?name=$module_name&file=submit\" "
			."method=\"post\" NAME=\"calendar\">";
	...
	echo "<br>("._CALAREYOUSURE.")</font><br><br>"
			...
			."<input style=\"font-weight: bold\" type=\"submit\" name=\"op2\" value="._CALOK
...
function PreviewStory($name, $address, $subject, $day, $month, $year, $endday, $endmonth, $endyear, $story, $topic, $type, $startHour, $startMin, $startampm, $endHour, $endMin, $endampm, $alldayevent, $barcolor) {
	...
	echo "<table width=\"70%\" bgcolor=\"#ffffff\" cellpadding=\"0\" cellspacing=\"1\" border=\"0\"align=\"center\"><tr><td>"
			."<table width=\"100%\" bgcolor=\"#ffffff\" cellpadding=\"8\" cellspacing=\"1\" border=\"0\"><tr><td>";
	echo ""
			."<b>"._CALEVENTDATEPREVIEW."</b>:&nbsp;"
			."$month/$day/$year<br><br><p align=center><img src=imagemevento.jpg border=1></p><br>";
...
function SubmitStory($name, $address, $subject, $day, $month, $year, $endday, $endmonth, $endyear, $story, $topic, $type, $startHour, $startMin, $startampm, $endHour, $endMin, $endampm, $alldayevent, $barcolor) {
  ...
  $diretorio = "./loadeventos/";
  $arquivo = str_replace(" ", "_", $foto1["name"]); 
  $arquivo = strtolower($arquivo); 
  $arquivo = $diretorio . $arquivo;
  $arquivo = $diretorio . $foto1["name"]; 
  $retarda = $foto1["name"]; 
  if (move_uploaded_file($foto1['tmp_name'], $arquivo)) { 
    echo ""; 
} 
else { 
    echo ""; 
	...
	if($postdirect) {
		$result = mysql_query("insert into $prefix"._events." values (NULL, '$retarda', '$subject', now(), '$story', '0', '0', '$topic','$name','$newDate','$endDate','$startTime','$endTime', '$alldayevent', '$barcolor')");
		if(!$result) {
		...
		$result = mysql_query("insert into $prefix"._events_queue." values (NULL, '$uid', '$retarda', '$subject', '$story', now(), '$topic', '$newDate','$endDate','$startTime','$endTime','$alldayevent', '$barcolor')");
		if(!$result) {
			echo mysql_errno(). ": ".mysql_error(). "<br>";
			exit();
		}
