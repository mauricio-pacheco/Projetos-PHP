	...
	if ($allowcomments) {
		...
		if (sql_num_rows($result3) == 0) {
			...
			while(list($cid, $eid, $comment, $date, $name) = sql_fetch_array($result3)) {
				...
				if (is_admin($admin)) {
					$out .= "&nbsp;[ <a href=\"modules.php?name=$module_name&op=CalendarRemoveComments&cid=$cid&eid=$eid\">"
								._CALCOMMENTDELETE."</a> ]\n";
				}
...
function buildDay($Date) {
	...
	if ($Date_Array[2] < 1970) {
		...
		print("\n\t\t<TABLE width=\"100%\" bgcolor=\"$trimbgcolor\" border=1 cellspacing=0><TR><TD><FONT color=\"$trimtextcolor\" SIZE=2><b>Nomeações</b></font></TD></TR></TABLE>");
...
elseif ($barcolor == "w") $caltype=$calcat4;
	...
	echo "<br><p align=center><img src='http://www.codemau.org.br/loadeventos/$aid'><p><b>"._CALARTICLETEXT."</b>:<br>$body<p></td>"
			."<td valign=\"middle\"></td>"
			."</tr>"
			."</table>"
			."<br><br>"
			."<font size=1>"._CALPOSTEDBY.""._CALPOSTEDON." $Date_Array2[1]/$Date_Array2[2]/$Date_Array2[0] ";
if ($postdirect =="0") ;
echo "</font></center>";
	if (is_admin($admin)) {
		echo "<center>[ "
	...
	if (($user != $anonymous & $user == $informant & $isadmin != "1")) {
		echo "<center>[ <a href=\"modules.php?name=Calendar&op=CalendarRemoveStory&eid=$eid\">"._DELETE."</a> ]</center>\n";
	}
	// END
...
function CalendarIndex($Date,$type,$eid) {
	...
	if (!(($eventsopeninnewwindow) && ($type == "view"))) {
		include("header.php");
		OpenTable();
    echo "<center><img src='tcalendario.jpg'></center>\n";
	CloseTable();
		...
		echo ""._CALTYPES.":&nbsp;&nbsp;$calcat1&nbsp;<img src=\"images/calendar/ballr.gif\">&nbsp;&nbsp;&nbsp;$calcat2&nbsp;<img src=\"images/calendar/ballg.gif\">&nbsp;&nbsp;&nbsp;$calcat3&nbsp;<img src=\"images/calendar/ballb.gif\">&nbsp;&nbsp;&nbsp;$calcat4&nbsp;<img src=\"images/calendar/ballw.gif\">&nbsp;&nbsp;&nbsp;$calcat5&nbsp;<img src=\"images/calendar/bally.gif\"><br><br>";
		if ($useInternationalDates) {
			echo "<font size=\"2\">IR PARA A SEGUINTE PÁGINA:&nbsp;<select name=\"jumpday\">";
