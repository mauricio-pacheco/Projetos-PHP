...
function navbar($sid, $title, $thold, $mode, $order) {
    ...
    echo "<tr><td bgcolor=\"#ffffff\" align=\"center\" width=\"100%\">\n";
    if ($anonpost==1 OR is_admin($admin) OR is_user($user)) {
		echo "
		<form id=form1 name=form1 method=post action='javaScript:window.print()'>
            <input type=submit name=Submit value='Imprimir Notícia' />
        </form>
		<form action=\"modules.php?name=$module_name&amp;file=comments\" method=\"post\">"
		    ."<input type=\"hidden\" name=\"pid\" value=\"$pid\">"
		    ."<input type=\"hidden\" name=\"sid\" value=\"$sid\">"
		    ."<input type=\"hidden\" name=\"op\" value=\"Reply\">"
		    ."<input type=\"submit\" value=\""._REPLYMAIN."\"></td></form></tr>";
    }
    echo "<tr><td bgcolor=\"#ffffff\" align=\"center\"><font class=\"tiny\">"._COMMENTSWARNING."</font></td></tr></table>"
...
function DisplayKids ($tid, $mode, $order=0, $thold=0, $level=0, $dummy=0, $tblwidth=99) {
	...
	while ($row = $db->sql_fetchrow($result)) {
		...
		if ($r_email) {
		    echo "<img src=comentado.gif>&nbsp;Comentário Postado por <b>$r_subject</b> <font class=\"content\">";
		    if(!$cookie[7]) {
			echo "("._SCORE." $r_score";
		...
		if($r_score >= $thold) {
		    ...
		    if (!eregi("[a-z0-9]",$r_subject)) $r_subject = "["._NOSUBJECT."]";
		    formatTimestamp($r_date);
		    echo "<img src=resposta.gif>&nbsp;<font class=\"content\" color=\"$textcolor2\"><a href=\"modules.php?name=$module_name&amp;file=comments&amp;op=showreply&amp;tid=$r_tid&amp;sid=$r_sid&amp;pid=$r_pid&amp;mode=$mode&amp;order=$order&amp;thold=$thold#$r_tid\">Resposta de $r_subject - "._ON." $datetime</font></a><br>";
...
function DisplayTopic ($sid, $pid=0, $tid=0, $mode="thread", $order=0, $thold=0, $level=0, $nokids=0) {
	...
	if ($email) {
	    echo "<img src=comentado.gif>&nbsp;Comentário Postado por <b>$subject</b> <font class=\"content\">";
	    if(!$cookie[7]) {
		echo "("._SCORE." $score";
...
function singlecomment($tid, $sid, $mode, $order, $thold) {
    ...
    echo "<table width=\"99%\" border=\"0\"><tr bgcolor=\"$bgcolor1\"><td width=\"500\">";
    formatTimestamp($date);
    if($email) echo "<img src=comentado.gif>&nbsp;Comentário Postado por <b>$subject</b> <font class=\"content\" color=\"$textcolor2\">("._SCORE." $score)<br>"._BY." <a href=\"mailto:$email\"><font color=\"$bgcolor2\">$name</font></a> <font class=content><b>($email)</b></font> "._ON." $datetime";
    else echo "<img src=comentado.gif>&nbsp;Comentário Postado por <b>$subject</b> <font class=content>("._SCORE." $score)<br>"._BY." $name "._ON." $datetime";
...
function reply($pid, $sid, $mode, $order, $thold) {
	...
	if ($subject == "") $subject = "["._NOSUBJECT."]";
	formatTimestamp($date);
	echo "<img src=comentado.gif>&nbsp;<img src=comentado.gif>&nbsp;Comentário Postado por <b>$subject</b> <font class=\"content\">";
	if (!$temp_comment) echo"";
	if ($email) {
	...
	echo "<font class=\"option\"><b>Nome:</b></font><br>";
	if (!eregi("Re:",$subject)) $subject = "Re: ".substr($subject,0,81)."";
	echo "<input type=\"text\" name=\"subject\" size=\"50\" maxlength=\"85\"><br><br>";
...
# $Log: comments.php,v $
# Revision 1.4  2004/12/08 00:22:34  chatserv
# http://www.nukefixes.com/ftopicp-3479.html#3479
#
# Revision 1.3  2004/11/29 22:52:11  chatserv
# http://www.nukefixes.com/ftopict-1098.html
#
