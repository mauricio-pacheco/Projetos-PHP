...
function process_smilies($message, $url_prefix = '')
{
    ...
    if (!isset($orig)) {
        ...
        for($i = 0; $i < count($smilies); $i++) {
            ...
            $repl[] = '<img src="' . $url_prefix . $CPG_M_DIR . '/images/smiles' . '/' . ($smilies[$i][1]) . '" alt="' . ($smilies[$i][2]) . '" border="0" />';
        } 
    } 
...
function generate_smilies($form = 'post', $field = 'message')
{
    ...
    foreach($smilies as $smiley) {
        ...
        $html .= '                <td width="5%"><a href="javascript:emoticon_' . $form . '(\'' . $smiley[0] . '\')"><img src="' . $CPG_M_DIR . '/images/smiles/' . $smiley[1] . '" alt="' . $caption . '" width="15" height="15" border="0" title="' . $caption . '"></a></td>' . "\n";
    } 
