...
function form_yes_no($text, $catname)
{
    ...
    if ($catname == 'uploads' && USER_ADMIN_MODE) {
...
{
    global $CONFIG, $ALBUM_DATA, $album, $lang_modifyalb_php, $CPG_M_DIR;
...
Pic[0] = '$CPG_M_DIR/images/nopic.jpg'

EOT;

    $initial_thumb_url = $CPG_M_DIR . '/images/nopic.jpg';
...
{
    global $CONFIG, $USER_DATA, $ALBUM_DATA, $lang_modifyalb_php, $field_user_id, $field_user_name;
    ...
    if (GALLERY_ADMIN_MODE) {
        ...
        if ($ALBUM_DATA['category'] > FIRST_USER_CAT) {
            $result = db_query("SELECT $field_user_name FROM {$CONFIG['TABLE_USERS']} WHERE $field_user_id='" . ($ALBUM_DATA['category'] - FIRST_USER_CAT) . "'");
            if (mysql_num_rows($result)) {
                $user = mysql_fetch_array($result);
                $options[$ALBUM_DATA['category']] = sprintf($lang_modifyalb_php['owner_only'], $user[$field_user_name]);
            } 
        } 
        ...
        $options = array(0 => $lang_modifyalb_php['public_alb'],
            ...
            $USER_DATA['group_id'] => sprintf($lang_modifyalb_php['groupp_only'], $USER_DATA['group_name'])
            );
    } 
    echo <<<EOT
        <tr>
...
{
    global $CONFIG, $album, $PHP_SELF, $CPG_URL, $field_user_name, $field_user_id;
...
echo <<<EOT
        ...
        <form method="post" action="$CPG_URL&file=db_input">
