...
function list_users()
{
    ...
    for ($i = 0; $i < $row_count; $i++) {
        ...
        // NEW gtroll
        $avatar = $user['avatar'];
        if (!ereg("blank.gif", $avatar) && !ereg("gallery/blank.gif", $avatar) && $avatar != "" && $avatar) {
            if (ereg( "(http)", $avatar) ) {
        ...
        // END NEW gtroll
            ...
            if (mysql_num_rows($result)) {
                ...
                if ($CONFIG['thumb_method']=='none'){
                    $user_thumb = "<img src=\"" . get_pic_url($picture, 'fullsize') . "\" {$image_size['geom']} alt=\"".$user[$field_user_name]."\" border=\"0\" class=\"image\">"; // $user[$field_user_name]
                }else{
    ...
    for ($alb_idx = 0; $alb_idx < $disp_album_count; $alb_idx++) {
        ...
        if ($visibility == '0' || $visibility == (FIRST_USER_CAT + USER_ID) || $visibility == $USER_DATA['group_id'] || USER_IS_ADMIN) {
            if ($count > 0) { // Inserts a thumbnail if the album contains 1 or more images
                ...
                if ($CONFIG['thumb_method']=='none'){
	            	$alb_list[$alb_idx]['thumb_pic'] = "<img src=\"" . get_pic_url($picture, 'fullsize') . "\" {$image_size['geom']} alt=\"".$alb_thumb['title']."\" border=\"0\" class=\"image\">";    
            	}
            	else{
...
global $BREADCRUMB_TEXT, $meta_link, $thisalbum ;
