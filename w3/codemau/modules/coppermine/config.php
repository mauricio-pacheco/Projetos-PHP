...
require($CPG_M_DIR . '/include/sql_parse.php');
...
function form_img_pkg($text, $name)
{
    global $CONFIG;
    $value = $CONFIG[$name];
    $none_selected = ($value == 'none') ? 'selected' : '';
    ...
    echo <<<EOT
        ...
        <td class="tableb" valign="top">
                        <select name="$name" class="listbox">
                        		<option value="none" $none_selected>Nenhum</option>
...
// for postnuke change
        ...
        msg_box($lang_config_php['info'], $lang_config_php['upd_success'], $lang_continue, $CPG_URL . '&file=config');
        pagefooter();
    ...
    pageheader($lang_config_php['title']);
    msg_box($lang_config_php['info'], $lang_config_php['restore_success'], $lang_continue, $CPG_URL . '&file=config');
    pagefooter();
    include('footer.php');
} 
pageheader($lang_config_php['title']);
...
echo <<<EOT
    <form action="$CPG_URL&file=config" method="post">
