...
EOT;
    ...
    $lb .= '                        <option value="0"' . ($highlight == 0 ? ' selected': '') . ">* Nenhuma Categoria Cadastrada *</option>\n";
...
EOT;
    ...
    while ($picture = mysql_fetch_array($results)) {
	    ...
	    if ($CONFIG['thumb_method']=='none'){
		    $thumb_url = get_pic_url($picture, 'fullsize');
		}else{
...
{
    global $CAT_LIST, $opp, $CPG_URL, $CPG_M_DIR;
    ...
    foreach ($CAT_LIST3 as $key => $category) {
        ...
        if ($category['pos'] > 0) {
            echo '                <td class="tableb" width="4%"><a href="' . $CPG_URL . '&file=catmgr&oppe=move&cid1=' . $category['cid'] . '&pos1=' . ($category['pos']-1) . '&cid2=' . $category['prev'] . '&pos2=' . ($category['pos']) . '">' . '<img src="' . $CPG_M_DIR . '/images/up.gif"  border="0">' . '</a></td>' . "\n";
        } else {
            echo '                <td class="tableb" width="4%">' . '&nbsp;' . '</td>' . "\n";
        } 

        if ($category['pos'] < $category['cat_count']-1) {
            echo '                <td class="tableb" width="4%"><a href="' . $CPG_URL . '&file=catmgr&oppe=move&cid1=' . $category['cid'] . '&pos1=' . ($category['pos'] + 1) . '&cid2=' . $category['next'] . '&pos2=' . ($category['pos']) . '">' . '<img src="' . $CPG_M_DIR . '/images/down.gif"  border="0">' . '</a></td>' . "\n";
        } else {
            echo '                <td class="tableb" width="4%">' . '&nbsp;' . '</td>' . "\n";
        } 

        if ($category['cid'] != 1) {
            echo '                <td class="tableb" width="4%"><a href="' . $CPG_URL . '&file=catmgr&oppe=deletecat&cid=' . $category['cid'] . '" onClick="return confirmDel(\'' . addslashes(str_replace('&nbsp;', '', $category['catname'])) . '\')">' . '<img src="' . $CPG_M_DIR . '/images/delete.gif"  border="0">' . '</a></td>' . "\n";
        } else {
            echo '                <td class="tableb" width="4%">' . '&nbsp;' . '</td>' . "\n";
        } 

        echo '                <td class="tableb" width="4%">' . '<a href="' . $CPG_URL . '&file=catmgr&oppe=editcat&cid=' . $category['cid'] . '">' . '<img src="' . $CPG_M_DIR . '/images/edit.gif" border="0">' . '</a></td>' . "\n";
        echo '                <td class="tableb" width="4%">' . "\n" . cat_list_box($category['parent'], $category['cid']) . "\n" . '</td>' . "\n";
        echo "        </tr>\n";
    } 
} 

$opp = isset($HTTP_POST_VARS['opp']) ? $HTTP_POST_VARS['opp'] : '';
...
switch ($opp) {
    ...
    case 'updatecat':
        if (!isset($HTTP_POST_VARS['cid']) || !isset($HTTP_POST_VARS['parent']) || !isset($HTTP_POST_VARS['catname']) || !isset($HTTP_POST_VARS['description'])) cpg_die(CRITICAL_ERROR, sprintf($lang_catmgr_php['miss_param'], 'updatecat'), __FILE__, __LINE__);

        $cid = (int)$HTTP_POST_VARS['cid'];
        $parent = (int)$HTTP_POST_VARS['parent'];
        $catname = trim($HTTP_POST_VARS['catname']) ? $HTTP_POST_VARS['catname'] : '&lt;???&gt;';
        $description = $HTTP_POST_VARS['description'];
//        $catname = trim($HTTP_POST_VARS['catname']) ? addslashes($HTTP_POST_VARS['catname']) : '&lt;???&gt;';
//        $description = addslashes($HTTP_POST_VARS['description']);
    ...
    case 'createcat':
        if (!isset($HTTP_POST_VARS['parent']) || !isset($HTTP_POST_VARS['catname']) || !isset($HTTP_POST_VARS['description'])) cpg_die(CRITICAL_ERROR, sprintf($lang_catmgr_php['miss_param'], 'createcat'), __FILE__, __LINE__);
...
$oppe = isset($HTTP_GET_VARS['oppe']) ? $HTTP_GET_VARS['oppe'] : '';
