...
function displayNewGall($row) {
	global $imagepath;
	if($row[new_day] == 0) {
		return "<img src=\"$imagepath/newred.gif\" border=\"0\" width=\"18\" height=\"17\" alt=\""._GALNEWMEDIA1.'">';
	}
	elseif($row[new_day] < 3) {
		return "<img src=\"$imagepath/newred.gif\" border=\"0\" width=\"18\" height=\"17\" alt=\"".sprintf(_GALNEWMEDIA2, $row[new_day]).'">';
	}
...
function showgall($gid, $picnum, $orderby) {
	...
	if ($ok) {
		...
		while($row = $db->sql_fetchrow($result)) {
			...
			print "<div align=\"center\">[ $val_size px]<br>[ $val_hits ]</div>\n";
			print '</td></tr></table>';
			//CloseTable();
			print '</td>'."\n";
