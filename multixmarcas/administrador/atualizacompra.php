...
$sql = "UPDATE cw_compras SET status = '$status' WHERE id = '$idcompra'";
if(mysql_query($sql)) {

echo "<script>alert('COMPRA ATUALIZADA COM SUCESSO!')</script>";
echo "<script>location.href='detalhecompra.php?id=$idcompra&idcliente=$idcliente&data=$data&hora=$hora'</script>";
