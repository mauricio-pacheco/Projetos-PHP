...
function theindex($new_topic=0) {
	...
	if ($fullcount > 0 OR $c_count > 0 OR $articlecomm == 0 OR $acomm == 1) {
	    $morelink .= "<b><table><tr><td>$story_link<img src=\"amnot.gif\" border=\"0\"></a></td><td>&nbsp;$story_link"._READMORE."</b></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>   ";
	} else {
	    $morelink .= "";
	}
	if ($fullcount > 0) { $morelink .= ""; }
	if ($articlecomm == 1 AND $acomm == 0) {
	    if ($c_count == 0) { $morelink .= "<td>$story_link<img src=\"amnot.gif\" border=\"0\"></a></td><td>&nbsp;$story_link"._COMMENTSQ."</a></td></tr></table>"; } elseif ($c_count == 1) { $morelink .= "$story_link$c_count "._COMMENT."</a>"; } elseif ($c_count > 1) { $morelink .= "$story_link$c_count "._COMMENTS."</a>"; }
	}
	$sid = intval($s_sid);
	if ($catid != 0) {
