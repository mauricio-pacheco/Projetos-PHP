<?php

/**************************************************************************/
/* PHP-NUKE: Advanced Content Management System                           */
/* ============================================                           */
/*                                                                        */
/* This is the language module with all the system messages               */
/*                                                                        */
/* If you made a translation go to the my website and send to me          */
/* the translated file. Please keep the original text order by modules,   */
/* and just one message per line, also double check your translation!     */
/*                                                                        */
/* You need to change the second quoted phrase, not the capital one!      */
/*                                                                        */
/* If you need to use double quotes (") remember to add a backslash (\),  */
/* so your entry will look like: This is \"double quoted\" text.          */
/* And, if you use HTML code, please double check it.                     */
/**************************************************************************/
/* Translated to Brazilian Portuguese by:                                 */
/**************************************************************************/
/* Luiz Gustavo Aleagi Nunes - aleagi - tatau                             */
/* aleagi@phpnuke.org.br - aleagi@gmail.com.br                            */
/* Comunidade PHP-Nuke Brasil - CNB - http://phpnuke.org.br               */
/**************************************************************************/


define("_SEND","Enviar");
define("_FUNCTIONS","Funções");
define("_YES","Sim");
define("_NO","Não");
define("_CATEGORY","Menu");
define("_SAVECHANGES","Salvar alterações");
define("_PAGEBREAK","Se o texto que você for excrever for muito grande, divida-o em páginas usando <b>&lt;!--pagebreak--&gt;</b> para cada quebra de página.");
define("_SIGNATURE","Assinatura");
define("_DESCRIPTION","Descrição");
define("_TITLE","Título");
define("_ACTIVE","Ativo");
define("_DEACTIVATE","Desativar");
define("_INACTIVE","Inativo");
define("_ACTIVATE","Ativar");
define("_CONTENT","Conteúdo");
define("_ADDCATEGORY","Adicionar Menu");
define("_EDITCATEGORY","Editar Menu");
define("_ADD","Adicionar");
define("_LANGUAGE","Idioma");
define("_CONTENTMANAGER","Gerenciador de Conteúdo");
define("_DELCONTWARNING","Você tem certeza de que deseja apagar esta página?");
define("_DELCONTENT","Apagar página");
define("_CURRENTSTATUS","Status atual");
define("_ADDANEWPAGE","Adicionar uma nova Página");
define("_CSUBTITLE","Subtítulo");
define("_HEADERTEXT","Texto do Cabeçalho");
define("_PAGETEXT","Texto da página");
define("_FOOTERTEXT","Texto do rodapé");
define("_ACTIVATEPAGE","Ativar esta página?");
define("_EDITPAGECONTENT","Editar conteúdo da página");
define("_DELCONTENTCAT","<b>AVISO</b>: Você tem certeza de que deseja apagar esta Menu?<br><br>As páginas que estiverem dentro desta Menu, se existirem, serão removidas, porém, não serão anexadas a outra Menu.");
define("_DELCATEGORY","Apagar Menu");


?>