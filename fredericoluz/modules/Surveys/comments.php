...
function navbar($pollID, $title, $thold, $mode, $order) {
    ...
    if($title) {
	echo "<tr><td bgcolor=\"#ffffff\" align=\"center\"><font class=\"content\" color=\"$textcolor1\">\"$title\" ";
	    if(is_user($user)) {
    ...
    echo "<tr><td bgcolor=\"#ffffff\" align=\"center\"><font class=\"tiny\">"._COMMENTSWARNING."</font></td></tr>\n"
	  ."</table>"
...
function DisplayKids ($tid, $mode, $order=0, $thold=0, $level=0, $dummy=0, $tblwidth=99) {
	...
	while($row = $db->sql_fetchrow($result)) {
			...
			if($r_score >= $thold) {
				...
				if (!eregi("[a-z0-9]",$r_subject)) $r_subject = "["._NOSUBJECT."]";
				formatTimestamp($r_date);
				echo "<img src=resposta.gif>&nbsp;<font class=\"content\"><a href=\"modules.php?name=$module_name&amp;file=comments&amp;op=showreply&amp;tid=$r_tid&amp;pollID=$r_pollID&amp;pid=$r_pid&amp;mode=$mode&amp;order=$order&amp;thold=$thold#$r_tid\">Resposta de $r_subject - "._ON." $datetime</a></font><br>";
...
function DisplayTopic ($pollID, $pid=0, $tid=0, $mode="thread", $order=0, $thold=0, $level=0, $nokids=0) {
	...
	while ($count_times < $num_tid) {
		...
		if ($email) {
			...
			echo "<p><img src=comentado.gif>&nbsp;Comentário postado por <b>$subject</b> <font class=content>";
			if(!$cookie[7]) {
				echo "";
				if($reason>0) echo ", $reasons[$reason]";
				echo "";
...
function reply ($pid, $pollID, $mode, $order, $thold) {
	...
	echo "<br><br><font class=\"content\"><B>Nome:</B></FONT><BR>";
	if (!eregi("Re:",$subject)) $subject = "Re: ".substr($subject,0,81)."";
	echo "<INPUT TYPE=\"text\" NAME=\"subject\" SIZE=50 maxlength=85 ><BR>";
