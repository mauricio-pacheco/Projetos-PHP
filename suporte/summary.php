...
<?
			...
			$outpmenu .= "<fieldset><legend>&nbsp;<img src='crmlogosmall.gif'>&nbsp;&nbsp;Buscar Tarefas em Relatório&nbsp;";
...
//			$outpmenu .= '<iframe src="empty.html" name="uRail" id="uRail"></iframe>';
			...
			$outpmenu .= "<!--\n";
			$outpmenu .= "	if (document.summ.extended.value=='0') {\n";
			...
			$outpmenu .= "<td colspan=3><br><fieldset><table><tr><td>$lang[incldel]:</td><td><input type=checkbox class='radio'  name='includedeleted'$ins2> </td>";
			...
			$outpmenu .= "<tr><td>$lang[mustcontain]:</td><td><input type='text' class='searchx' name='wildsearch' value='$wildsearch'> </td></tr>";
...
//			ShowEntitiesOpen();
	...
	if (!in_array($e[eid],$printed)) {
		if (CheckEntityAccess($e['eid'])<>"nok") {
			...
			$outp .= "</td><td><b>$lang[category]</b></td><td><b>$lang[duedate]</b></td></tr>";
...
//			$remark .= "$tp[uur]:$tp[min]h.<br>";
			...
			if ($e[duedate]=="") {
				$e[duedate] = "<font color='FF3300'>nada</font>";	
			}					  
			...
			if ($e5= mysql_fetch_array($rult2)) {
				...
				$outp .= "<td><font color='FF3300'>nada</font></td>";
			}
			$outp .= "<td></tr>";
			if ($includecontents) {
				$outp .= "<tr><td></td><td>Conteúdos:</td></tr>";
				$cont = ereg_replace("\n","<BR>",$e[content]);
				$cont = trim($cont);
				if ($cont=="") {
					$cont = "-empty-";
	...
	//		}
			...
			$outp .= "<tr><td></td><td colspan=8 nowrap>";
			$outp .= " <img src='arrow.gif'>&nbsp;<a class='bigsort' title='Baixar Tarefa em PDF' href='sumpdf.php?pdf=$e[eid]' target='pdf'>PDF</a></td><td>$extra</td></tr>";	
			unset($extra);
			unset($ftel);
