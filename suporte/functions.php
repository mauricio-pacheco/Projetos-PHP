...
//										print "GH: -" . $gh[0] . "-";
						...
						if ($MainListColumnsToShow['cb_duedate']) {
							...
							if ($e[duedate]=="") {
									$e[duedate] = "<font color='FF3300'>Não Definida</font>";	
							}					  
...
// +=======================================================================================
					...
					print "<table><tr><td><fieldset><font color='#FF0000'>ALTERADO</font></fieldset></td></tr></table><br><br>";
...
//    UPDATE person SET password= encrypt('$newpassword1','$selectedusername') WHERE username='$selectedusername'";
			...
			print "<tr><td></td><td colspan=2>";
			print "<table border=0 width='100%'><tr><td colspan=2><fieldset><legend>&nbsp;<img src='crmlogosmall.gif'>&nbsp;&nbsp;Perfis dos Usuários&nbsp;</legend><table border='0' width='100%' cellspacing='0' cellpadding='4'>";
			print "<tr><td><br><form name='selectprofile' action='userprofile.php' method=post></td></tr><tr><td><select name='EditProfile'>";
			$la = GetUserProfiles();
			print "<option value='x'>- novo perfil -</option>";
			foreach($la AS $profile) {
	...
	?>
	  ...
	  <table border="0" cellpadding="0" cellspacing="3">
		...
		<tr> 
          ...
          <tr> 
            <td width="286" colspan=2> <a class='topnav' href="javascript:clearUser()" title='<? echo $lang[clearusername];?>'><font size=-2><? echo $lang[clearusername];?></font></a> </font> 
			</td></tr>
          <tr> 
          ...
          <tr> 
            <td width="286" colspan=2> <a class='topnav' href="javascript:clearPwd()" title='<? echo $lang[clearpassword];?>'><font size=-2><? echo $lang[clearpassword];?></font></a> </font> 
...
//								$result= @mcq($sql,$db);
			...
			if (sizeof($pass)>2) {
					?>
					<input type="submit" tabindex=4 value="ENTRAR" name="submit" OnClick="javascript: setCookie('repository',document.login.repository[document.login.repository.selectedIndex].value);CheckSecure()">
					<?
			} else {
					?>
					<input type="submit" tabindex=4 value="ENTRAR" name="submit" OnClick="javascript: setCookie('repository',document.login.repository[document.login.repository.selectedIndex].value);">
					<?
			}
			?>
...
function AvailableFormTags() {
			...
			<tr><td><a style='cursor: pointer' title='Click to copy to clipboard' onClick="CopyToClipboard('#ACTICON#');">#ACTICON#</a></td></tr>
...
function MainEditForm($e, $limited=false) {
	...
	//					print "<table border='100' cellpadding='3' cellspacing='0' bgcolor='#cfcfbb' width='90%'><tr><td align='left'>";
						...
						print "<img src='paper.gif' border='0'>&nbsp;$lang[editing] $lang[entity] <b>$e</b> $lang[ownedby] $owner, $lang[assignedto] $assignee.<br>";
						...
						print "<br><img src='tatu.gif' border='0'>&nbsp;$lang[curstat]: <font color='#E67300'>$ea[status]</font>, $lang[created] " . TransformDate($cdate) . ", $lang[lastupdate] " . TransformDate("$tp[dag]-$tp[maand]-$tp[jaar]") . " $tp[uur]:$tp[min]h.<br>";
						print "<br><img src='capone.gif' border='0'>&nbsp;$lang[category]: $ea[category] - $lang[priority]: <font color='#ff0000'>$ea[priority]</font>"; 
...
// - ------ LKG eid
			...
			<?
						...
						if ($e!="_new_" && CheckEntityAccess($e) == "ok") {
							...
							print "&nbsp;<a class='bigsort' style='cursor:pointer' title='Imprimir' href=\"javascript:popPDFprintwindow('sumpdf.php?pdf=$e&print=1')\"><img src='print.gif' style='border:0'> IMPRIMIR TAREFA</a>";
...
//										print "&nbsp;<input type='text' size=4 name='parent' value='" . $ea['parent'] . "'>&nbsp;";
	...
	// +=======================================================================================
						...
						print "<table border='0' cellpadding='3' cellspacing='0' bgcolor='' width='90%'><tr><td align='left'><fieldset><legend>&nbsp;&nbsp;<img src='crmlogosmall.gif'>&nbsp;&nbsp;$e&nbsp;</legend><table><tr><td>";
...
//							qlog("END");
				...
				print "<fieldset><legend>&nbsp;&nbsp;Dono&nbsp;</legend>";
	...
	//				print "Info on Cust#$ea[$GLOBALS[TBL_PREFIX]customer]";	
					...
					if ((strtoupper($GLOBALS['HideCustomerTab'])<>'YES') || (is_administrator())) {
						...
						print "</td><td align='right'>&nbsp;&nbsp;<a href='javascript:popcustomerjournal(" . $cust['id'] . ")' class='bigsort'><img src='journal.gif' style='border:0' title='Show journal'></a>&nbsp;";
					...
					print "\n<tr><td colspan=4>";
					print "<div style='display:block' id='open$cust[custname]'><a style='cursor:pointer' class='bigsort' OnClick=\"javascript:showLayer('rest$cust[custname]');hideLayer('open$cust[custname]');showLayer('close$cust[custname]');\" title='Expand'></div>";
...
//					print "<table width=100%><tr><td>" . AW_array("obj", $arr, $header) . "<br><br><br><br><br><br></td></tr></table>";
					...
					$toprint .= "<tr><td><fieldset><legend>&nbsp;&nbsp;$lang[listofatt]&nbsp;<br></legend><table border=0>";
					...
					$toprint.= "<br><td>$lang[filesize]</td><td>$lang[creationdate]</td><td>$lang[owner]</td><td>$lang[deletefile]</td></tr>";
					while ($files= mysql_fetch_array($result)) {
						$ownert = GetUserNameByName($files['username']);
						print "\n";
						unset($ins_rec1);				
						...
						if ($dis_link) {
							$toprint .= "<img src='anexo.gif'>&nbsp;$files[filename]";
							...
							$toprint .= "<img src='anexo.gif'>&nbsp;<a href=csv.php?fileid=$files[fileid] class='bigsort'>$files[filename]</a> $ins_rec1";
						}
				...
				if (CheckEntityAccess($e) == "ok") {
					print "<tr><td colspan=6><fieldset><legend>&nbsp;&nbsp;$lang[attachfile]&nbsp;</legend><INPUT TYPE='hidden' name='MAX_FILE_SIZE' value='52428800'><INPUT NAME='userfile' TYPE='file' $roins></fieldset></td></tr>";
				}
				...
				if ($nonavbar || $_REQUEST['nonavbar']) {
					if ($e=="_new_") {
						if (CheckEntityAccess($e) == "ok") {
							print "<table width='90%'><tr><td><br><input class='txt' type='button' OnClick=\"javascript:document.EditEntity.changed.value='0';CheckForm();\" name=sb value='$lang[saveclose] $showAlt' align='right' $roins><br>&nbsp;&nbsp;&nbsp;";
						}
					} else {
						if (CheckEntityAccess($e) == "ok") {
							print "<table width='90%'><tr><td><br><input class='txt' type='button' OnClick=\"javascript:document.EditEntity.changed.value='0';CheckForm();\" name=sb value='$lang[saveclose] $showAlt' align='right' $roins><br>&nbsp;&nbsp;&nbsp;";
					...
					if (CheckEntityAccess($e) == "ok") {
						print "<br><input class='txt' type='button' name=sb OnClick=\"javascript:document.EditEntity.changed.value='0';CheckForm();\" value='$lang[cancel]' align='right' OnClick='javascript:window.close();'></td></tr></table><br>";
					}
				} else {
					if (CheckEntityAccess($e) == "ok") {
						print "<table width='90%'><tr><td><br><input class='txt' type='button' name=sb OnClick=\"javascript:document.EditEntity.changed.value='0';CheckForm();\" value='$lang[save] $showAlt ' $roins><br>";
					}
					print "</td></tr></table>";
				}
				print "</form>";
