...
if ($Submitted && sizeof($email)<1 && $email[0]=="") {
			    print "Foram apagados olhares como todos os endereços de e-mail. A colocação de alarme inteira foi apagada.";
				$sql= "DELETE FROM $GLOBALS[TBL_PREFIX]calendar WHERE eID='$eID'";
				if ($debug) { print "\nSQL: $sql\n"; }
				mcq($sql,$db);
				journal($eID,"Colocação de alarme ");
...
if (!$eID) {
		print "Um erro ocorreu. Não há nenhuma eID.<br>";
		exit;
		...
		if (IsDate($dd)) {
			...
			<SCRIPT LANGUAGE="JavaScript" type="text/javascript">
		    <!--
				alert("A data que você enviou é inválida.");
		    //-->
		    </SCRIPT>
			<?
			$error = 1;
		...
		print "Alarme selecionado para $duedate. A advertência será enviada para os $emails.<br>";
...
//	$owner = $owner[name];
	...
	print "<fieldset>Ajustando a data do alarme para a entidade $eID possuído por $owner, assinado para $assignee, ";
	...
	print "<hr>";
		...
		if ($result= mysql_fetch_array($result)) {
		    print "<b><img src='error.gif'>&nbsp;Esta entidade tem já uma data do alarme! Você pode alterá-lo usando o formulário abaixo.</font></b>";
			$already = 1;
		} else {				
			print "<b>Opte por valores impressos que são os endereços do E-mail do cessionário e do proprietário desta entidade.</b>";
...
?>
<br><b>
Use uma vírgula (,) para seperarar mais de um endereço de e-mail, <br>apague um endereço para evitar enviar uma nota de alarme.
</b><br><br>
<?
print "<table>";
if (!$already) {
