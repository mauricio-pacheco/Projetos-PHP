<?php
	...
	case 'startCalendar':
		...
		echo '<div style="margin-bottom: 3px;">
					<form name="changeCalendarDate">
						<select class="manchete_texto9" class=caixacontato id="ccMonth" onChange="startCalendar($F(\'ccMonth\'), $F(\'ccYear\'))">';
				...
				echo '</select>
						&nbsp;
						<select class="manchete_texto9" class=caixacontato id="ccYear" onChange="startCalendar($F(\'ccMonth\'), $F(\'ccYear\'))">';
				...
				if($_COOKIE['nodstrumCalendarV2']) {
					...
					echo '&nbsp;&nbsp;<a href="#" onClick="showLoginBox();" class="manchete_texto9">Login</a>';
				}
	...
	case 'listEvents':
		...
		if($eventQuery) {
			if(mysql_num_rows($eventQuery) >0) {
				echo '<br><font color=#086E08>EVENTOS DO DIA:</font> '. date("d", $timeStamp) .'/'. date("m", $timeStamp) .'/'. date("Y", $timeStamp) .'<br><br>';
  ...
  <tr>
    <td width=40><img src=imagens/star.png>&nbsp;</td>
  ...
  <tr>
    <td><img src=imagens/branco.gif height=6></td>
								...
								<div style="font-size: 9px;">
									<span style="cursor: pointer;" class=manchete_texto9 onClick="deleteEvent('.$e_id.')">
										Apagar este evento
									</span>
								</div>
							</div>';
