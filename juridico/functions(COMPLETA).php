...
function qlog($msg) {
	...
	if ($GLOBALS['tracelogused'] == "") {
		...
		//GLOBALS['tracelog'] = date('U') . " " . $usrname . "Init::" . "Initializing CRM-CTT ... \n";
...
//				print string_r($pdfilterextrafield);
	...
	if (strtoupper($LetUserSelectOwnListLayout)=="YES") {
		...
		$customize = " <font size='-2'><img src='arrow.gif'>&nbsp;<a href='choose_cols.php?dothis=personal&cur=" . $CurURL . "' class='bigsort'>" . strtolower($lang['edit']) . "</a>&nbsp;</font>";
	}
...
//	print $query;
	...
	if ($filter=="custinsert" || GetClearanceLevel($GLOBALS['USERID'])=="ooro") {
		...
		if (GetClearanceLevel($GLOBALS['USERID']) <> "ro" && GetClearanceLevel($GLOBALS['USERID']) <> "ro+" && GetClearanceLevel($GLOBALS['USERID']) <> "read-only-all") {
			...
			$a = "<a class='bigsort' title='$lang[createreports]'  href='entityreport.php?stashid=" . $query_stashid . "'><img src='word.gif' style='border:0'></a>&nbsp;";
		}
	}

	$outputbuffer2 .= "<a class='bigsort' title='$lang[downloadsumcsv] (direct)'  href='csv.php?ListLayout=true&stashid2=" . $query_stashid . "&separator=RealExcel'><img src='excel_large.gif' style='border:0'></a>&nbsp;&nbsp;";
	
	if (GetClearanceLevel($GLOBALS['USERID']) <> "ro" && GetClearanceLevel($GLOBALS['USERID']) <> "ro+" && GetClearanceLevel($GLOBALS['USERID']) <> "read-only-all") {
		$outputbuffer2 .= "<a class='bigsort' title='$lang[downloadsumcsv] (choose fields)'  href='" . $csv_url . "'><img src='excel_large_double.gif' style='border:0'></a>&nbsp;&nbsp;";
	}
...
//			}
				...
				if ($e<>"_new_") {
					if ($ea[deleted]=='y') {
						print "<tr><td><input DISABLED type='checkbox' class='radio' name=deleted value='y' CHECKED $roins OnChange=\"AlertUser('IsChanged');\"> $lang[isdeleted]&nbsp;&nbsp;&nbsp;&nbsp;<a OnClick='pophelp(12)' class='bigsort' cursor='click' style='cursor: help'><img src='info.gif'></a>";
					} else {
						print "<tr><td><input DISABLED type='checkbox' class='radio' name=deleted value='y' $roins OnChange=\"AlertUser('IsChanged');\"> $lang[isdeleted]&nbsp;&nbsp;&nbsp;&nbsp;<a OnClick='pophelp(12)' class='bigsort' cursor='click' style='cursor: help'><img src='info.gif'></a>";
					}
				}
				if ($ea[waiting]=='y') {
						print "<tr><td><input DISABLED type='checkbox' class='radio' name=waiting value='y' CHECKED $roins OnChange=\"AlertUser('IsChanged');\"> $lang[iswaiting]&nbsp;&nbsp;&nbsp;&nbsp;<a OnClick='pophelp(10)' class='bigsort' cursor='click' style='cursor: help'><img src='info.gif'></a></td></tr>";
				} else {
						print "<tr><td><input DISABLED type='checkbox' class='radio' name=waiting value='y' $roins OnChange=\"AlertUser('IsChanged');\"> $lang[iswaiting]&nbsp;&nbsp;&nbsp;&nbsp;<a OnClick='pophelp(10)' class='bigsort' cursor='click' style='cursor: help'><img src='info.gif'></a></td></tr>";
				}
				if ($ea[obsolete]=='y') {
						print "<tr><td><input DISABLED type='checkbox' class='radio' name=obsolete value='y' CHECKED $roins OnChange=\"AlertUser('IsChanged');\"> $lang[doesntbelonghere]&nbsp;&nbsp;&nbsp;&nbsp;<a OnClick='pophelp(11)' class='bigsort' cursor='click' style='cursor: help'><img src='info.gif'></a></td></tr>";
				} else {
						print "<tr><td><input DISABLED type='checkbox' class='radio' name=obsolete value='y' $roins OnChange=\"AlertUser('IsChanged');\"> $lang[doesntbelonghere]&nbsp;&nbsp;&nbsp;&nbsp;<a OnClick='pophelp(11)' class='bigsort' cursor='click' style='cursor: help'><img src='info.gif'></a></td></tr>";
				}
// =========================================>>
				if ($ea[readonly]=='y') {
			...
			print "<tr><td colspan=6><fieldset><legend>&nbsp;<img src='crmlogosmall.gif'>&nbsp;&nbsp;$lang[attachfile]&nbsp;</legend><input TYPE='hidden' name='MAX_FILE_SIZE' value='52428800'><input NAME='userfile' TYPE='file'>&nbsp;&nbsp;&nbsp;&nbsp;<a OnClick='pophelp(9)' class='bigsort' cursor='click' style='cursor: help'><img src='info.gif'></a></fieldset></tr></td>";
...
function SendEmail($eid,$who,$NewOrChanged, $BodyData, $SubjectData, $FromName, $FromAddress) { 
	...
	if ($who=="owner") {
			...
			$receipient_name = "CRM Administrator";
...
//function LogContactMoment($eidcid,$type,$to,$meta,$body) {
	...
	if ($receipient_mail=="ARRAY") {
		for ($i=1;$i<sizeof($recp_list);$i++) {
			$mail->AddAddress($recp_list[$i],"CRM Administrator");				
		}
	} else { 
		$mail->AddAddress($receipient_mail,"CRM Administrator");	
	}
		...
		while ($ra = mysql_fetch_array($result)) {
			if ($ra[emailadress]) {
					...
					for ($y=0;$y<sizeof($t);$y++) {
						$mail->AddAddress(trim($t[$y]),"CRM Administrator");
...
//    UPDATE person SET password= encrypt('$newpassword1','$selectedusername') WHERE username='$selectedusername'";
			...
			$la = GetUserProfiles();
			print "<tr><td>Profile</td><td><select name='profile'>";
			print "<option value=''>- no profile -</option>";
			foreach($la AS $profile) {
				print "<option value='" . $profile[0] . "'>" . $profile[1] . "</option>";
			}
			print "</select></td></tr>";	
			
			print "<tr><td>Full name:</td><td><input size=50 type='text' name='FULLNAME' value='$result[FULLNAME]'></td></tr>";
			print "<tr><td>Daily " . $lang['briefover'] . " e-mail:</td><td><select name='dailymail'><option value='No'>No</option><option value='Yes' " . $ins . ">Yes</option></select></td></tr>";
			...
			if ($administrator=="yes" || $administrator=="Yes") { // Only print this option when the current user is an administrator
					...
					print "<option $a value='administrator'>Administrator</option>";
			}
			print "</select>";
			print "</td></tr>";
			...
			print "</select>&nbsp;<input type='submit' value='Select'><br><br></td></tr></table></fieldset></td></tr></table>";
			...
			print "<tr><td></td><td colspan=2>";
			print "<table border=0 width='100%'><tr><td colspan=2><fieldset><legend>&nbsp;<img src='crmlogosmall.gif'>&nbsp;&nbsp;User profiles&nbsp;</legend><table border='0' width='100%' cellspacing='0' cellpadding='4'>";
			print "<tr><td><br><form name='selectprofile' action='userprofile.php' method=post></td></tr><tr><td><select name='EditProfile'>";
			$la = GetUserProfiles();
			print "<option value='x'>- new profile -</option>";
			foreach($la AS $profile) {
			...
			print "<tr><td></td><td><fieldset><legend>&nbsp;<img src='crmlogosmall.gif'>&nbsp;&nbsp;Options&nbsp;</legend>";
			print "<br><img src='arrow.gif'>&nbsp;<a href='admin.php?ExportUsers=1' class='bigsort'> Export  accounts (CRM-CTT layout, encrypted)</a><br><br>";
			print "<img src='arrow.gif'>&nbsp;<a href='admin.php?ImportUsers=1' class='bigsort'> Import accounts (CRM-CTT layout, encrypted)</a><br><br>";
			print "<img src='arrow.gif'>&nbsp;<a href='admin.php?ImportCSVUsers=1&tib=users' class='bigsort'> Import accounts (Plain text CSV file)</a><br><br>";
			print "<img src='arrow.gif' style=border:0> <a href='admin.php?SuperListUsers=1' class='bigsort'>&nbsp;User activity statistics</a> <IMG SRC='graph.gif' style=border:0><br><br></fieldset>";
			print "</td></tr>";
		...
		print "<TR><TD><a href='javascript:popUserActivityGraph(". $user_to_edit . ")' class='bigsort'><IMG SRC='graph.gif' style=border:0></a></TD></TR>";
		print "<tr><td nowrap>Account $lang[name]:</td><td><input type='text' name='bla' value='$result[name]' DISABLED></td></tr>";
		print "<tr><td nowrap>Account $lang[password]: <br>($lang[passwarn])</td><td><input type='password' name='accpass1' value=''></td></tr>";
		print "<tr><td nowrap>Account $lang[password]:</td><td><input type='password' name='accpass2' value=''></td></tr>";
		...
		if ($result[CLLEVEL]=="ro" || $result[CLLEVEL]=="ro+") {
			...
			if ($result['PROFILE']=="Default" || $result['PROFILE']=="") {
				...
				*/
				...
				print "<tr><td>Allow user to set the status:</td><td><select name='n_HIDEADDTAB'><option value='n' " . $ins2 . ">Yes</option><option value='y' " . $ins . ">No</option></select></td></tr>";
				...
				print "<tr><td>Allow user to set the priority:</td><td><select name='n_HIDECUSTOMERTAB'><option value='n' " . $ins2 . ">Yes</option><option value='y' " . $ins . ">No</option></select></td></tr>";
				...
				print "<tr><td>Allow user to set the due-date:</td><td><select name='n_HIDECSVTAB'><option value='n' " . $ins2 . ">Yes</option><option value='y' " . $ins . ">No</option></select></td></tr>";
			...
			print "<tr><td nowrap>Full name:</td><td><input size=50 type='text' name='FULLNAME' value='$result[FULLNAME]'></td></tr>";
			print "<tr><td>E-mail:</td><td><input type='text' size=50 name='EMAIL' value='$result[EMAIL]'></td></tr>";
			...
			if ($result['PROFILE']=="Default" || $result['PROFILE']=="") {
				...
				print "<tr><td>Limit to " . $lang['customer'] . "-numbers:<br>(entities of other customers cannot be seen)</td><td><input type='text' name='n_LIMITTOCUSTOMERS' value='" . $result['LIMITTOCUSTOMERS'] . "'> (;-separated list of customer-numbers)</td></tr>";
			...
			if ($result["administrator"]=="yes" && $administrator<>"yes") {
				...
				if (is_administrator()) { // Only print this option when the current user is an administrator
						...
						print "<option SELECTED value='administrator'>Administrator</option>";				
				}
			...
			print "<input type='button' name='cancel' value='Back' OnClick=\"document.location='admin.php?userman=1&password=$password';\">";
			print "</form></td></tr>";
	...
	if (strstr($result1['setting'],"COLOR")){ 
		...
		*/
		...
		if ((stristr($result1['setting'],"show") ||stristr($result1['setting'],"DISPLAYNUMSUMINMAINLIST")||stristr($result1['setting'],"CHECKFORDOUBLEADDS")||stristr($result1['setting'],"USE_EXTENDED_CACHE") || stristr($result1['setting'],"enable") || $result1['setting']=="DisplayNOToptioninfilters" || stristr($result1['setting'],"ALSO_PR") || stristr($result1['setting'],"auto") || stristr($result1['setting'],"CAL_USE") || stristr($result1['setting'],"BLOCK") || stristr($result1['setting'],"force") || stristr($result1['setting'],"letuser") || stristr($result1['setting'],"hide") || stristr($result1['setting'],"USEWAITINGANDDOESNTBELONGHERE") || stristr($result1['setting'],"FormFinity") || stristr($result1['setting'],"PDF") || stristr($result1['setting'],"langover")) && !stristr($result1['setting'],"recentedit") && ($result1['setting']<>"MonthsToShow") && ($result1['setting']<>"FORCEDFIELDSTEXT")) {
			...
			print "</TD><TD><BR><SELECT NAME='data'><OPTION " . $yes . ">Yes</OPTION><OPTION " . $no . ">No</OPTION>" . $option_insert . "</TD></TR>";
...
<a OnClick='pophelp(16)' class='bigsort' style='cursor: help'><img src='info.gif'></a>
...
<?
	...
	?>
		<tr><td NOWRAP><img src='arrow.gif'>&nbsp;<a class='bigsort' title='' href='profile.php?<?echo $epoch;?>'><? echo GetUserName($GLOBALS['USERID']);?>'s Profile</a>&nbsp;&nbsp;&nbsp;&nbsp;</td><td NOWRAP><? echo $lang[username];?>/<? echo strtolower($lang[password]);?>/email</td></tr>	
	<?
	}
	?>
		<tr><td NOWRAP><img src='arrow.gif'>&nbsp;<a class='bigsort' title='' href='stats.php?<?echo $epoch;?>'><? echo $lang[maninfo];?></a>&nbsp;&nbsp;&nbsp;&nbsp;</td><td NOWRAP><? echo $lang[maninfoverb];?></td></tr>	

		<?

			print "<tr><td NOWRAP><img src='arrow.gif'>&nbsp;<a class='bigsort' title='' href='admin.php?info=1&$epoch'>$lang[administration]</a>&nbsp;<img src='lock.png'></td><td NOWRAP>$lang[administrationverb] (alt-a)</td></tr>";
		//}
		...
		if (strtoupper($GLOBALS['EnableEntityReporting'])=="YES") {
			print "<tr><td NOWRAP><img src='arrow.gif'>&nbsp;<a class='bigsort' title='' href='entityreport.php?$epoch'>$lang[createreports]</a></td><td NOWRAP>$lang[entreportverbose]</td></tr>";
		}
...
//			print $query;
		...
		$export_t = "<a href='javascript:popExportWindow(\"" . $id . "\");' class='bigsort' title='Use results for Excel/CSV download'><img style='border:0' src='excel_large.gif'></a>&nbsp;&nbsp;";
		$export_t .= "<a href=\"javascript:popPDFwindow('customers.php?pdf=1&close=1&stashid=" . $id . "');\" class='bigsort' title='Use results for PDF report'><img style='border:0' src='pdf.gif'></a>&nbsp;&nbsp;";

		if ($GLOBALS['EnableMailMergeAndInvoicing']) {
			$export_t .= "<a title='Use results for mail merge' href=\"javascript:poplittlewindow('invoice.php?little=1&stashid=" . $id . "&mm=1')\"><img style='border:0' src='word.gif'></a>&nbsp;";

			$export_t .= "<a title='Use results for e-mail merge' href=\"javascript:poplittlewindow('customers.php?stashid=" . $id . "&mm=1')\"><img style='border:0' src='mail2.gif'></a>";
		}
		print "</legend>";
		...
		if ($CustomerListColumnsToShow['cb_active']) {
			if ($_REQUEST['DESC']) {
				print "<td><a href='customers.php?stashid=" . $id_no . "&sortorder=" . base64_encode("customer.active") . "' class='bigsort'><b>Active</b></td>";
			} else {
				print "<td><a href='customers.php?stashid=" . $id_no . "&DESC=1&sortorder=" . base64_encode("customer.active DESC") . "' class='bigsort'><b>Active</b></td>";
			}
		}
...
function prtsrc2($tim) {
	...
	if (!$tim==1) {
		...
		print "<td>CID:</td><td><input type='text' name='c_id' value='' size=4><input type='hidden' name='det' value='1'></td></tr><tr><td colspan=2><input type='submit' value='Show'></td></tr></table>";
		print "<input type='hidden' name='uitgeklapt' value=''>";
		print "<br><br><img src='arrow.gif'>&nbsp;<a href='customers.php?nonavbar=$nonavbar&add=1' class='bigsort'>$lang[addcust]</a><br><img src='arrow.gif'>&nbsp;<a href='javascript:popExportWindow(\"" . $store . "\");' class='bigsort'>Export</a><br><img src='arrow.gif'>&nbsp;<a 	href='javascript:showLayer(\"Advanced\");' OnClick='document.pb.uitgeklapt.value=1;' class='bigsort'>More</a>&nbsp;&nbsp;";
...
function MainCalendar() {
		...
		$MainPageCalendar=1;
		print "<tr><td><table border='0'><tr><td><fieldset><legend>&nbsp;<img src='crmlogosmall.gif'>&nbsp;&nbsp;Calendar&nbsp;</legend>"; 	
...
function RecentEntities() {
		...
		print "<tr><td NOWRAP><fieldset><legend>&nbsp;<img src='crmlogosmall.gif'>&nbsp;&nbsp;$fullname's recent " . strtolower($lang[entities]) . "</legend><table width='100%' border=0>";
...
global $lang,$host,$user,$pass,$database,$table_prefix,$QUERY_STRING;
  ...
  <?
	  DisplayCSS();
	  ?>
  <TITLE>CRM-CTT - <? echo $title; ?></TITLE>
  <META HTTP-EQUIV="Expires" CONTENT="0">
  <META HTTP-EQUIV="Cache-Control" CONTENT="no-cache">
  <META HTTP-EQUIV="PRAGMA" CONTENT="NO-CACHE">
  <META NAME="Description" CONTENT="CRM-CTT - Online collaboration tool for any department where something comes in, must be handled, and goes out.">
  <META NAME="Author" CONTENT="http://www.crm-ctt.com">
  ...
  <SCRIPT LANGUAGE="javascript" type="text/javascript">
	...
	//-->
		...
		?>
					...
					<table width=100% border=0><tr><td><table border=0><tr><td><fieldset><legend>&nbsp;&nbsp;
	<?
		print "<a href='http://www.crm-ctt.com' target='_new' title=\"$CRM_VERSION\"><img alt='$CRM_VERSION' style='border:0' src='crm_small.png'></a>"; 
	?>
	  ...
	  <table border="0" cellpadding="0" cellspacing="3">
		...
		<tr> 
            <td nowrap colspan=2>
              <font color="#000000"> 
              <? echo $err_string; ?> &nbsp;&nbsp;</b><a OnClick='pophelp(17)' class='topnav' style='cursor: help'><img src='info.gif'></a>
              </font>
          ...
          <tr> 
			...
			<legend>&nbsp;<img src='crmlogosmall.gif'>&nbsp;&nbsp;Log on&nbsp;</legend><table>
			<tr><td><? echo $lang[username];?><br>
              <input type="text" name="name" tabindex=1>
            </td>
          </tr>
          <tr> 
            <td width="286" colspan=2> <a class='topnav' href="javascript:setUser()" title='<? echo $lang[saveusername];?>'><font size=-2><? echo $lang[saveusername];?></font></a><font size=-2> | <a class='topnav' href="javascript:clearUser()" title='<? echo $lang[clearusername];?>'><font size=-2><? echo $lang[clearusername];?></font></a> </font> 
			</td></tr>
          <tr> 
          ...
          <tr> 
            <td width="286" colspan=2> <a class='topnav' href="javascript:setPwd()" title='<? echo $lang[savepassword];?>'><font size=-2><? echo $lang[savepassword];?></font></a><font size=-2> | <a class='topnav' href="javascript:clearPwd()" title='<? echo $lang[clearpassword];?>'><font size=-2><? echo $lang[clearpassword];?></font></a> </font> 
...
function PrintFooter() {
		global $CRM_VERSION;
	//	print "<table><tr><td><a href='http://www.crm-ctt.com' target='new' class='bigsort'>&copy;&nbsp; $CRM_VERSION</a></td></tr></table>";
}
...
function AvailableFormTags() {
			...
			<tr><td><img src='graph.gif' border=1> Activity graph</td><td><a style='cursor: pointer' title='Click to copy to clipboard' onclick="CopyToClipboard('#ACTICON#');">#ACTICON#</a></td></tr>
			<tr><td><img src='word.gif' style='border:0'> Entity RTF Report</td><td><a style='cursor: pointer' title='Click to copy to clipboard' onclick="CopyToClipboard('#REPORTICON#');">#REPORTICON#</a></td></tr>
...
function ProcessTriggers($onchange,$eid,$to_value,$log=false) {
	...
	*/
	...
	RealMail($template,$eid,$customer,GetUserEmail($GLOBALS['USERID']),GetUserName($GLOBALS['USERID']),GetCustomerEmail($customer),"0",$subject,$attach,$filename,$report_attach);
			...
			RealMail($template,$eid,$customer,$GLOBALS['admemail'],"CRM Administrator",GetUserEmail($entity['assignee']),"0",$subject,$attach,$filename,$report_attach);
			...
			RealMail($template,$eid,$customer,$GLOBALS['admemail'],"CRM Administrator",GetUserEmail($entity['owner']),"0",$subject,$attach,$filename,$report_attach);
			...
			RealMail($template,$eid,$customer,$GLOBALS['admemail'],"CRM Administrator",$GLOBALS['admemail'],"0",$subject,$attach,$filename,$report_attach);
			if ($eid<>0) {
			...
			RealMail($template,$eid,$customer,$GLOBALS['admemail'],"CRM Administrator",GetUserEmail($user),"0",$subject,$attach,$filename,$report_attach);
...
//	if ($report_attach=="PDF") {
	...
	if ($template) {
		...
		if ($report_attach=="PDF" && $entity) {
			...
			$pdf_title_link = "http://www.crm-ctt.com";
...
function AdminTabs($navid=false) {
	...
	$tabbs["main"] = array("admin.php" => "<b>Main</b>", "comment" => "In this section you'll find the main administrative functions like System Configuration, Data management, and import/export.");
	$tabbs["sys"] = array("admin.php?password=$password&sysval=1" => "Global system values", "comment" => "All main system values can be adjusted in this section");
	$tabbs["users"] = array("admin.php?password=&adduser=1&userman=1" => "Users & profiles", "comment" => "In this section you can manage your users and user profiles.");
	$tabbs["ef"] = array("extrafields.php?tabletype=entity&ti=1" => "Extra fields", "comment" => "Go to this section to add new fields to the entity or customer table.");
	$tabbs["triggers"] = array("trigger.php?trig=1" => "Event triggers/workflow", "comment" => "Use this this page to configure events. An event can cause a trigger to fire. A trigger will do something for you, like sending an e-mail, or adjust entity values.");
	$tabbs["templates"] = array("admin.php?templates=1" => "Templates", "comment" => "Upload and edit your templates in this section. Templates are used throughout CRM-CTT to display reports, forms and pages.");
	$tabbs["customtabs"] = array("customtabs.php?ovw=1" => "Custom navigation tabs", "comment" => "In this section you can add and manage extra tabs. Tabs are used to navigate through CRM-CTT.");
...
// - ------ LKG eid
			...
			<?
						...
						if ($e!="_new_" && CheckEntityAccess($e) == "ok") {
							...
							// PDF download icon
							print "<br><br><a href='javascript:popActivityGraph(" . $e .");' title='Show activity graph'><img src='graph.gif' border=1></a>&nbsp;&nbsp;";
							if (strtoupper($GLOBALS['EnableEntityJournaling'])=="YES") {
								print "<a class='bigsort' title='Journal' href='javascript:popjournal($e);'><img src='journal.gif' style='border:0'></a>&nbsp;";
							}
...
//										print "&nbsp;<input type='text' size=4 name='parent' value='" . $ea['parent'] . "'>&nbsp;";
							...
							if (strtoupper($GLOBALS['EnableEntityReporting'])=="YES") {
								...
								print "&nbsp;<a title='Generate RTF report' href=\"javascript:poplittlewindow('entityreport.php?SingleEntity=" . $e . "')\"><img src='word.gif' style='border:0'></a>";
							}
						}
						print "</td></tr>";
	...
	// +=======================================================================================
				...
				if (strtoupper($GLOBALS['AutoCompleteCustomerNames'])<>"YES") {
					print "<fieldset><legend>&nbsp;&nbsp;$lang[customer]&nbsp;</legend>";
...
//							qlog("END");
							...
							print "\n<select name=CRMcustomer size='1' $roins OnChange=\"javascript:AlertUser('IsChanged');\">";
							$sql= "SELECT * FROM $GLOBALS[TBL_PREFIX]customer WHERE active='yes' ORDER BY custname";
							$result= mcq($sql,$db);
				...
				if ($GLOBALS['USERID']==$ea['owner'] || $e=="_new_") {
					if (EmailTriggerForOwnerSet($e)==true) {
						...
						print "<input type='hidden' name='notify_owner_posted' value='1'><input style='border:0' type='checkbox' name='notify_owner' value='1' > E-mail updates";
					}
				}
				print "</fieldset></td><td valign='top'>";
				print "<fieldset><legend>&nbsp;&nbsp;$lang[assignee]&nbsp;</legend>";

				if (CheckEntityAccess($e) == "readonly") {
					print "<table cellspacing=4 cellpadding=2><tr><td nowrap>" . GetUserName($ea['assignee']) . "</td></tr></table>";
				} else {
					print "<select name='assigneeNEW' size='1' $roins $roins2 OnChange=\"javascript:AlertUser('IsChanged');\">";
				...
				if ($GLOBALS['USERID']==$ea['assignee'] || $e=="_new_") {
					if (CheckEntityAccess($e) == "ok") {
						if (EmailTriggerForAssigneeSet($e)==true) {
							print "<input type='hidden' name='notify_assignee_posted' value='1'><input style='border:0' type='checkbox' name='notify_assignee' value='1' CHECKED> E-mail updates";
						} else {
							print "<input type='hidden' name='notify_assignee_posted' value='1'><input style='border:0' type='checkbox' name='notify_assignee' value='1' > E-mail updates";
						}
					}
				}
	...
	//				print "Info on Cust#$ea[$GLOBALS[TBL_PREFIX]customer]";	
					...
					print "<div style='display:block' id='open$cust[id]'><a style='cursor:pointer' class='bigsort' OnClick=\"javascript:showLayer('rest$cust[id]');hideLayer('open$cust[id]');showLayer('close$cust[id]');\" title='Expand'>&nbsp;&nbsp;<img src='arrow.gif'> more</a></div>";

					
					print "<div style='display:none' id='close$cust[id]'><a style='cursor:pointer' class='bigsort' OnClick=\"javascript:hideLayer('rest$cust[id]');showLayer('open$cust[id]');hideLayer('close$cust[id]')\" title='Collapse'>&nbsp;&nbsp;<img src='arrow.gif'> less</a></div>";
					print "</td><td valign=top>";
					...
					if ((strtoupper($GLOBALS['HideCustomerTab'])<>'YES') || (is_administrator())) {
						...
						print "</td><td align='right'>&nbsp;<a href='javascript:popActivityCustomerGraph(" . $cust['id'] .");' title='Show activity graph'><img src='graph.gif' border=1></a>&nbsp;&nbsp;<a href='javascript:popcustomerjournal(" . $cust['id'] . ")' class='bigsort'><img src='journal.gif' style='border:0' title='Show journal'></a>&nbsp;";
						
						$stashid = PushStashValue("SELECT * FROM $GLOBALS[TBL_PREFIX]customer WHERE id=" . $cust['id']);

						print "<a href=\"javascript:popPDFwindow('customers.php?pdf=1&stashid=" . $stashid . "')\" class='bigsort'><img src='pdf.gif' style='border:0' title='Show PDF'></a>";
					...
					print "\n<tr><td colspan=4>";
					print "<div style='display:block' id='open$cust[custname]'><a style='cursor:pointer' class='bigsort' OnClick=\"javascript:showLayer('rest$cust[custname]');hideLayer('open$cust[custname]');showLayer('close$cust[custname]');\" title='Expand'>&nbsp;&nbsp;<img src='arrow.gif'> more</a></div>";

					
					print "<div style='display:none' id='close$cust[custname]'><a style='cursor:pointer' class='bigsort' OnClick=\"javascript:hideLayer('rest$cust[custname]');showLayer('open$cust[custname]');hideLayer('close$cust[custname]')\" title='Collapse'>&nbsp;&nbsp;<img src='arrow.gif'> less</a></div> ";
...
//					print ExtraFieldsBox($enetjes,$readonly_lock,"A");
			//	}
				if ($GLOBALS['DisableCommentField'] <> "Yes") {
					...
					print "<a OnClick='InsertDateTime();' class='bigsort' style='cursor:pointer' title='Insert date, time and your name'><img src='timedate.gif'></a>";
					print "<img src='decrease.gif' height='16' width='16' onclick='decreaseNotesHeight(document.EditEntity.content, 50);' style='cursor:pointer' title='Make Notes field smaller'>";
					print "<img src='increase.gif' height='16' width='16' onclick='increaseNotesHeight(document.EditEntity.content, 50);' style='cursor:pointer' title='Make Notes field bigger'><br>";
					...
					if (GetClearanceLevel($GLOBALS['USERID'])=="ro+" || ($limited == true)) {
						...
						print "$lang[attachfile]&nbsp;<INPUT TYPE='hidden' name='MAX_FILE_SIZE' value='52428800'><INPUT NAME='userfile' TYPE='file' >&nbsp;&nbsp;&nbsp;&nbsp;<a OnClick='javascript:pophelp(9)' class='bigsort' cursor='click' style='cursor: help'><img src='info.gif'></a>";
		...
		//		with the help of kaveh@arkasoft.com - select for history
				...
				print "<table><tr><td><br><fieldset><table>";
					...
					if ($e<>"_new_") {
						if ($ea[deleted]=='y') {
							print "<tr><td><input type='hidden' name='deleted_posted' value='1'><input type='checkbox' class='radio' name=deleted value='y' CHECKED $roins $roins2 OnChange=\"javascript:AlertUser('IsChanged');\"> $lang[isdeleted]&nbsp;&nbsp;&nbsp;&nbsp;<a OnClick='javascript:pophelp(12)' class='bigsort' cursor='click' style='cursor: help'><img src='info.gif'></a>";
						} else {
							print "<tr><td><input type='hidden' name='deleted_posted' value='1'><input type='checkbox' class='radio' name=deleted value='y' $roins $roins2 OnChange=\"javascript:AlertUser('IsChanged');\"> $lang[isdeleted]&nbsp;&nbsp;&nbsp;&nbsp;<a OnClick='javascript:pophelp(12)' class='bigsort' cursor='click' style='cursor: help'><img src='info.gif'></a>";
						}
					}
				if ($GLOBALS['UseWaitingAndDoesntBelongHere'] <> "No") {
					if ($ea[waiting]=='y') {
							print "<tr><td><input type='hidden' name='waiting_posted' value='1'><input type='checkbox' class='radio' name=waiting value='y' CHECKED $roins OnChange=\"javascript:AlertUser('IsChanged');\"> $lang[iswaiting]&nbsp;&nbsp;&nbsp;&nbsp;<a OnClick='javascript:pophelp(10)' class='bigsort' cursor='click' style='cursor: help'><img src='info.gif'></a></td></tr>";
					} else {
							print "<tr><td><input type='hidden' name='waiting_posted' value='1'><input type='checkbox' class='radio' name=waiting value='y' $roins OnChange=\"javascript:AlertUser('IsChanged');\"> $lang[iswaiting]&nbsp;&nbsp;&nbsp;&nbsp;<a OnClick='javascript:pophelp(10)' class='bigsort' cursor='click' style='cursor: help'><img src='info.gif'></a></td></tr>";
					}
					if ($ea[obsolete]=='y') {
							print "<tr><td><input type='hidden' name='obsolete_posted' value='1'><input type='checkbox' class='radio' name=obsolete value='y' CHECKED $roins OnChange=\"javascript:AlertUser('IsChanged');\"> $lang[doesntbelonghere]&nbsp;&nbsp;&nbsp;&nbsp;<a OnClick='javascript:pophelp(11)' class='bigsort' cursor='click' style='cursor: help'><img src='info.gif'></a></td></tr>";
					} else {
							print "<tr><td><input type='hidden' name='obsolete_posted' value='1'><input type='checkbox' class='radio' name=obsolete value='y' $roins OnChange=\"javascript:AlertUser('IsChanged');\"> $lang[doesntbelonghere]&nbsp;&nbsp;&nbsp;&nbsp;<a OnClick='javascript:pophelp(11)' class='bigsort' cursor='click' style='cursor: help'><img src='info.gif'></a></td></tr>";
					}
				}
	...
	// =========================================>>
					...
					if ($ea['private']=='y') {
							...
							print "<tr><td><input type='hidden' name='private_posted' value='1'><input type='checkbox' class='radio' name='private' value='y' $roins OnClick='javascript:document.EditEntity.private.style.background=\"#FF0000\"' OnChange=\"javascript:AlertUser('IsChanged');\"> Private</td></tr>";
					}
...
//					print "<table width=100%><tr><td>" . AW_array("obj", $arr, $header) . "<br><br><br><br><br><br></td></tr></table>";
				...
				if (CheckEntityAccess($e) == "ok") {
					print "<tr><td colspan=6><fieldset><legend>&nbsp;&nbsp;$lang[attachfile]&nbsp;</legend><INPUT TYPE='hidden' name='MAX_FILE_SIZE' value='52428800'><INPUT NAME='userfile' TYPE='file' $roins>&nbsp;&nbsp;&nbsp;&nbsp;<a OnClick='javascript:pophelp(9)' class='bigsort' cursor='click' style='cursor: help'><img src='info.gif'></a></fieldset></tr></td></tr>";
				}
...
function ParseCustomerFormTemplate($fileid,$cid=false,$formaction='index.php?logout=1',$readonly=true, $c_add="NO") {
	...
	if (CheckCustomerAccess($cid) <> "ok" && CheckCustomerAccess($cid) <> "readonly") {
		...
		$activityicon = "<a href='javascript:popActivityCustomerGraph(" . $cid .");' title='Show activity graph'><img src='graph.gif' border=1></a>";
		...
		if ($GLOBALS['EnableMailMergeAndInvoicing']) {
			$reporticon = "<a title='Use results for mail merge' href=\"javascript:poplittlewindow('invoice.php?little=1&stashid=" . $sid . "&mm=1')\"><img style='border:0' src='word.gif'></a>&nbsp;";
		} else {
			$reporticon = "";
		}
...
//		$assigneebox = "<select name='assigneeNEW' size='1' $roins $roins2 OnChange=\"javascript:AlertUser('IsChanged');\">";
		...
		$reporticon = "<a title='Generate RTF report' href=\"javascript:poplittlewindow('entityreport.php?SingleEntity=" . $eid . "')\"><img src='word.gif' style='border:0'></a>";
		...
		$acticon = "<a href='javascript:popActivityGraph(" . $e .");' title='Show activity graph'><img src='graph.gif' border=1></a>";
...
//		$cancelbutton = "<input type='submit' value='" . $lang['save'] . "'>";
		...
		if (GetClearanceLevel($GLOBALS['USERID'])=="ro+" && $eid<>"_new_") {
			...
			$addcommentbox .="$lang[attachfile]&nbsp;<INPUT TYPE='hidden' name='MAX_FILE_SIZE' value='52428800'><INPUT NAME='userfile' TYPE='file' >&nbsp;&nbsp;&nbsp;&nbsp;<a OnClick='javascript:pophelp(9)' class='bigsort' cursor='click' style='cursor: help'><img src='info.gif'></a>";
	...
	if ($pa[CLLEVEL]=="ro" || $pa[CLLEVEL]=="ro+") {
		...
		*/
		...
		if ($pa['administrator']=='yes') {
			...
			print "<tr><td>Daily " . $lang['briefover'] . " e-mail</td><td><select name='dailymail'><option value='No' $ins2>No</option><option value='Yes' " . $ins . ">Yes</option></select></td></tr>";
	}
...
function ShowAuthHeadersWebdav($cust=" - login") {
	global $title;
	$title .= $cust;
	header('WWW-Authenticate: Basic realm="CRM-CTT ' . $title . ' WebDav"');
...
function ShowAuthHeadersRSS($cust=" - login") {
	global $title;
	$title .= $cust;
	header('WWW-Authenticate: Basic realm="CRM-CTT ' . $title . ' RSS Feed"');
    header('HTTP/1.0 401 Unauthorized');
	echo "Access denied.";
}
...
//		print_r($arr);
		...
		<rss version="2.0">
		<channel>
		<title>CRM-CTT <? echo htmlspecialchars($GLOBALS['title']);?> <? echo htmlspecialchars($name);?></title> 
		<link><? echo $link;?></link> 
...
When set to ëYesí, customers which have an account are able to add entities by themselves. See<br>
also chapter ìUser typesî.<br>
